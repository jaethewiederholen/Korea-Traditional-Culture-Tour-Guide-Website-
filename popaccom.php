...
<center><h2> 사람들이 많이 찾는 숙소는 어디일까요? </h2></center>
...
<section class="gallery-block cards-gallery">
    <div class="row">
        ...
        <center><h3>전체</h3></center>
            <?php
            $sql=mq("select a.*, count
            from (SELECT room, count(*) count
                FROM `search` where not room is null group by room order by count desc limit 5) s
            left join accommodation a
            on s.room=a.room order by count desc");
        ...
        <center><h3>남성</h3></center>
            <?php
                $sql=mq("select a.*, gender, count
                from (select gender, room, count(*) count
                      from search s
                      left join members m on s.id=m.id where room is not null group by gender, room) s
                left join accommodation a
                on s.room=a.room where gender = 'male' order by count desc limit 5");
        ...
        <center><h3>여성</h3></center>
            <?php
                    ...
                    from (select gender, room, count(*) count
                          ...
                          left join members m on s.id=m.id where room is not null group by gender, room) s
                    left join accommodation a
                    on s.room=a.room where gender = 'female' order by count desc limit 5");
