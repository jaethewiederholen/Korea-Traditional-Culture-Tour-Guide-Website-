...
<center><h2> 사람들이 많이 찾는 음식점은 어디일까요? </h2></center>
...
<section class="gallery-block cards-gallery">
    <div class="row">
        ...
        <center><h3>전체</h3></center>
            <?php
            $sql=mq("select r.*, count
            from (SELECT food, count(*) count
                FROM `search` where not food is null group by food order by count desc limit 5) s 
            left join restaurant r
            on s.food=r.food order by count desc");
        ...
        <center><h3>남성</h3></center>
            <?php
                $sql=mq("select r.*, gender, count
                from (select gender, food, count(*) count
                      from search s 
                      left join members m on s.id=m.id where food is not null group by gender, food) s
                left join restaurant r
                on s.food=r.food where gender = 'male' order by count desc limit 5");
        ...
        <center><h3>여성</h3></center>
            <?php
                    ...
                    from (select gender, food, count(*) count
                          ...
                          left join members m on s.id=m.id where food is not null group by gender, food) s
                    left join restaurant r
                    on s.food=r.food where gender = 'female' order by count desc limit 5");
